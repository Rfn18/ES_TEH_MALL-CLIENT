...
const DetailTransaksi = ({
  ...
  const standName = (standId: string) => {
    return standList?.filter((s) => s.kd_stand === standId)[0]?.nama_stand;
  };
...
export const TransactionList = ({ standList }: { standList: StandForm[] }) => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      setTransactions(res.data.data.datas.data);
    } catch (e) {
  ...
  return (
    <>
      ...
      <div className="flex flex-wrap items-center gap-3 mt-6">
        <select
          className="border border-[#119184]/20 bg-[#f9fafb] rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-[#119184]/50 text-sm text-[#2f524a]"
          value={filters.stand}
      ...
      <div className="relative w-full overflow-x-auto border border-[#119184]/20 rounded-xl mt-4">
        <table className="table-fixed w-full text-[#2f524a]">
          ...
          <tbody className="font-semibold text-sm">
            {loading ? (
              <tr>
                <td colSpan={6} className="text-center py-16 text-[#2f524a]/40">
                  Memuat data...
                </td>
              </tr>
            ) : empty ? (
              ...
              filteredData.map((t) => (
                ...
                >
                  <td className="px-4 py-3 text-[#119184]">{t.no_transaksi}</td>
                  <td className="px-4 py-3">{standName(t.stand_id)}</td>
